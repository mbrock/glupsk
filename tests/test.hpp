...
inline constexpr std::string_view test_root_name = "glupsk";
...
inline bool path_starts_with(const std::vector<int>& path,
...
inline test_result& find_or_add_result(std::vector<test_result>& siblings,
                                       std::string_view name,
...
inline test_result* find_result(std::vector<test_result>& results,
                                const std::vector<int>& path) {
...
inline void collect_selected_leaves(const std::vector<test_result>& results,
...
inline void print_test_result(const test_result& result) {
    ...
    std::print("\x1b[2m{}\x1b[0m  ", format_path(result.path));
    if (!result.children.empty()) {
        std::print("\x1b[1m{}{}\x1b[0m", result.name, status);
    } else {
        std::print("{}{}", result.name, status);
    }
    if (!duration.empty()) {
        std::print(" {}", duration);
    }
...
inline void print_report() {
    std::println("\x1b[1m{}\x1b[0m", test_root_name);
...
inline void print_summary() {
    ...
    if (tests_failed == 0) {
        std::println("\n\x1b[32m+\x1b[0m all {} passed",
                     plural(tests_run, "test"));
        return;
    }

    std::print("\n\x1b[31m-\x1b[0m {} failed", plural(tests_failed, "test"));
    if (failures != tests_failed) {
        std::print(" with {} failed", plural(failures, "expectation"));
    }
    std::println(" out of {}", tests_run);
}

inline void note_progress(bool failed) {
    if (!progress_started) {
        progress_started = true;
        std::print("\x1b[2mrunning tests...\x1b[0m");
    }

    std::print(" {}", failed ? "\x1b[31mx\x1b[0m" : ".");
}
...
struct test_case {
    ...
    void operator=(F&& f) const {
        ...
        const auto tests_failed_before = tests_failed;
        const auto count_this_test =
        ...
        try {
            ...
            if (phase == run_phase::execution) {
                ++failures;
                std::println(stderr, "{}: unexpected exception: {}",
                             result.name, e.what());
            }
        } catch (...) {
            if (phase == run_phase::execution) {
                ++failures;
                std::println(stderr, "{}: unexpected non-std exception",
                             result.name);
            }
        }
...
struct expectation {
    ...
    ~expectation() {
        if (!ok && !explained) {
            std::println(stderr, "expectation failed");
        }
    }
    ...
    expectation& operator<<(T&& msg) {
        if (!ok) {
            explained = true;
            std::println(stderr, "{}", std::forward<T>(msg));
        }
        return *this;
    }
};
...
inline bool configure_filters(run_options options) {
    ...
    for (auto i = 1; i < options.argc; ++i) {
        ...
        if (!filter) {
            std::println(stderr, "invalid test selector: {}", options.argv[i]);
            return false;
        }
...
inline void run_test_leaf(const std::vector<int>& path) {
    ...
    const auto failures_before = failures;
    const auto tests_run_before = tests_run;
    const auto tests_failed_before = tests_failed;
...
inline int run(run_options options = {}) {
    reset_run_state();
    ...
    if (progress_started) {
        std::println("\x1b[2m done\x1b[0m\n");
    } else {
        std::println("");
    }
    print_report();
    print_summary();
    return tests_failed == 0 && (!filters.empty() ? tests_run != 0 : true) ? 0
                                                                           : 1;
}
